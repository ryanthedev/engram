...
- **Mark-and-sweep for full harvests** — stamp one `harvest_id` per run, then `KnowledgeDelete` the not-current rows for that collection+source (those whose `harvest_id != currentHarvestID`, per Plan 1's predicate). A partial/failed run must sweep NOTHING (fail-safe). **Deletion is ONLY via this sweep** — see the Build-time correction below: Plan 1's frozen API has no per-doc-id delete, so `FullHarvest` sources handle removals via the sweep and `Incremental` (additive) sources do not delete in v1.
...
---
## Notes
- **⚠ Build-time correction (2026-07-11, verified during P1/P2):** Plan 1's frozen knowledge API exposes **no per-doc-id delete** — `KnowledgeDelete` is *only* the mark-and-sweep predicate (`collection+source`, `harvest_id != current`), and the store refuses an empty `harvest_id` to prevent a full wipe. The plan's original "explicit per-file/per-record deletes" for incremental sources (`Sink.Delete(id)`, OAI `status="deleted"`, github per-file) is therefore **unimplementable against the API and was removed.** Revised, API-compatible model, applied in code and superseding the affected phase bodies:
  - `Sink` = `{Add, Flush}` (no `Delete`); `StateStore`/`Deps.State` removed (no v1 consumer); `Deps` now carries a `*slog.Logger`. This also makes the harvester **fully stateless** (Approach A's local state file is unnecessary once git-diff is dropped).
  - **github-repos → `FullHarvest` + sweep** (P4): re-ingest all glob-matched files each run under `source="github-repos"`; deleted files are removed by the post-run sweep. Shallow-clone keeps it cheap; the SHA-diff/skip optimization is deferred (it conflicts with per-source sweep granularity).
  - **arxiv-oaipmh → `Incremental` additive, no sweep** (P3): `status="deleted"` records are logged and skipped (documented v1 limitation — a withdrawn paper ingested via OAI is not removed in real time; reconciled only if engram later gains a per-id delete). Kaggle's sweep only covers `source="arxiv-kaggle"` rows.
  - Deletion semantics for P2's Runner: sweep only after a clean `FullHarvest`; `Incremental` sources never sweep. (P2 DW-2.2 already encodes this.)
